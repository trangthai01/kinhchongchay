...
class UploadsService
{
    ...
    public function legacyStorePost(CreateBlogEtcPostRequest $request)
    {
        ...
        $this->handleTags($request->get('post_tags'), $new_blog_post);
    ...
    public function legacyUpdatePost(UpdateBlogEtcPostRequest $request, $blogPostId)
    {
        ...
        $this->handleTags($request->get('post_tags'), $post);
    ...
    public function handleTags($tags, Post $post){
        ...
        if (!empty($tags)) {
            ...
            $postTags = Tag::whereIn('name', $tagList)->get()->pluck('id');
