...
Route::group(['middleware' => ['web'], 'namespace' => '\WebDevEtc\BlogEtc\Controllers'], static function () {
    ...
    Route::group(['prefix' => config('blogetc.blog_prefix', 'blog')], static function () {
        ...
        Route::get('/danh-muc/{categorySlug}', 'PostsController@showCategory')->name('blogetc.view_category');
    ...
    Route::group(['prefix' => config('blogetc.story_prefix', 'blog')], static function () {
        Route::get('/', 'PostsController@storyIndex')->name('storyetc.index');
    });
    ...
    Route::group(['prefix' => config('blogetc.admin_prefix', 'blog_admin')], static function () {
        ...
        Route::group(['prefix' => 'products'], static function () {
            Route::get('/', 'Admin\ManageProductsController@index')->name('blogetc.admin.products.index');

            Route::get('/add_product', 'Admin\ManageProductsController@create')->name('blogetc.admin.products.create');
            Route::post('/add_product', 'Admin\ManageProductsController@store')->name('blogetc.admin.products.store');

            Route::get('/edit_product/{productId}', 'Admin\ManageProductsController@edit')->name('blogetc.admin.products.edit');
            Route::patch('/edit_product/{productId}', 'Admin\ManageProductsController@update')->name('blogetc.admin.products.update');
